...
public class BlockScaffolding extends BlockBlock implements IWaterLoggable {
    ...
    private void updateNeighbours(World world, BlockPos pos){
        ...
        if(world.getBlockState(pos.up   ()).getBlock() == StairKeeper.BLOCK_SCAFFOLDING.get()) searchNeighbour(world, pos.up   ());
        if(world.getBlockState(pos.down ()).getBlock() == StairKeeper.BLOCK_SCAFFOLDING.get()) searchNeighbour(world, pos.down ());
    }
